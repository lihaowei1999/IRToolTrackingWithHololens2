...
def DetectBalls(img,_minSize,_maxSize,distanceTolerance,pre_retval=0,pre_labels=[],pre_stats=[],pre_centroids=[],pre_labelcorr=[]):
    ...
    # if previous frame considered
    _label_assign={}
    _maxAssigned=max(pre_labelcorr)
    re_retval=0
    re_labels=labels
    re_stats=[]
    re_centroids=[]
    re_corr=[]
    _distanceMatrx=np.zeros([retval,pre_retval])
    for i in range(retval):
        for j in range(pre_retval):
            _distanceMatrx[i,j]=np.sqrt(np.sum((centroids[i]-pre_centroids[j])**2))
    for i in range(pre_retval):
        min_val=np.min(_distanceMatrx)
        pos_min=np.where(_distanceMatrx==min_val)
        if min_val > distanceTolerance:
            break
        
        try:
            _distanceMatrx[:,int(pos_min[1][0])]=999
            _distanceMatrx[int(pos_min[0][0]),:]=999
            _label_assign[pre_labelcorr[int(pos_min[1][0])]]=_relabel[int(pos_min[0][0])]
        except Exception as e:
            print(e)
            print(_distanceMatrx)
            print(min_val)
            print(pos_min)
    for i in _relabel:
        if not i in _label_assign.values():
            _maxAssigned+=1
            _label_assign[_maxAssigned]=i
    # relabel and sort according sample id
    for i in sorted(_label_assign):
        re_retval+=1
        re_labels[labels==_label_assign[i]]=i
        re_stats.append(stats_ori[_label_assign[i]])
        re_centroids.append(list(centroids_ori[_label_assign[i]]))
        re_corr.append(i)

    # print("ctf")
    # print(re_centroids)
    return [re_retval,re_labels,np.array(re_stats),np.array(re_centroids),np.array(re_corr)]
...
class AHAT_NDIToolScene:
    def __init__(self,TolSide=6,TolAvg=5):
        ...
        Prime_list_file=loadmat("Prime_list_500.mat")
