...
class ToolDefDataCollection(QThread):
    SignalCollectionPercentage=pyqtSignal(tuple)
    def __init__(self,logger,SnapShotFrame,Num,Path):
        super(ToolDefDataCollection,self).__init__()
        self.logger=logger
        self.SnapShotFrmae=SnapShotFrame
        self.TotalNum=Num
        self.CurrFrame=0
        self.CaptureInterv=5
        self.FaultFrame=0
        self.FaultFrameTolerance=80
        self.Path=Path
        self.CollectData=[]
        self._start=False
    ...
    def run(self):
        ...
        while True:
            if not self._start:
                self.SignalCollectionPercentage.emit((0,))
                return
            
            if self.FaultFrame>=self.FaultFrameTolerance:
                self.logger.error("SnapShot not start")
                return
            
            if self.CurrFrame>=self.TotalNum:
                with QMutexLocker(self._startmutex):
                    self._start=False
                np.save(self.Path,self.CollectData)
                self.logger.info("Data Saved : "+self.Path)
                self.SignalCollectionPercentage.emit((0,))

            if self.SnapShotFrmae.empty():
                self.FaultFrame+=1
                QThread.msleep(self.CaptureInterv)
                continue

            SnapFrame=self.SnapShotFrmae.get()
            self.CollectData.append(SnapFrame)
            print(SnapFrame)
            self.CurrFrame+=1
            self.FaultFrame=0
            self.SignalCollectionPercentage.emit((int(self.CurrFrame/self.TotalNum*100),))
            QThread.msleep(self.CaptureInterv)
    ...
    def stop(self):
        with QMutexLocker(self._startmutex):
            self._start=False
            self.logger.info("End Data Collecting")
...
class SingleSensorCollectThread(QThread):
    def __init__(self,logger,SensorDisplayFrame,_pth,Sensorname):
        super(SingleSensorCollectThread,self).__init__()
        self.logger=logger
        self.DisplayFrame=SensorDisplayFrame
        self._pth=_pth

        self.Sensorname=Sensorname
        self.savepath=os.path.join(self._pth,(self.Sensorname+".npy"))
        self.data=[]
        self.continueFalseEnum=0
        self.FalseTolerance=100
        self._start=False
        self._startMutex=QMutex()
    ...
    def run(self):
        with QMutexLocker(self._startMutex):
            self._start=True
            self.logger.info("Sensor "+self.Sensorname+" Start recording")
            
        while True:
            if not self._start:
                self.logger.info("Sensor "+self.Sensorname+" Stop recording, frame num: "+str(len(self.data)))
                return 
            if not self.DisplayFrame.empty():
                curr_fr=self.DisplayFrame.get()
                self.continueFalseEnum=0
                self.data.append(curr_fr)
            else:
                self.continueFalseEnum+=1
            QThread.msleep(20)
            if self.continueFalseEnum>self.FalseTolerance:
                self.logger.error("Sensor "+self.Sensorname+" No Video")
                self.SaveSensorData()
                
    def SaveSensorData(self):
        self.logger.info("Sensor "+self.Sensorname+" Saving Data to: "+self.savepath)
        np.save(self.savepath,self.data)
